...
def split_train_and_test_set(measurements,
    ...
    """
    # clean data
    measurements.drop(['id'], axis=1, inplace=True)
    measurements.drop(['name'], axis=1, inplace=True)
    ...
    mag = np.zeros(len(measurements))
    '''for i_source in range(len(measurements)):
        # for debug, add by YH.
        a = measurements['source']
        b = measurements[i_source]
        c = (b == a)
        d = measurements.loc[c]
        e=d.magnitude
        f=e.iloc[0]
        ###########################################for debug, add by YH.
        mag[i_source] = measurements.loc[
            measurements['source'] == measurements[i_source]].magnitude.iloc[0]'''
    # mag[mag < 85] = 1
    # mag[mag >= 85] = 0
    mag = measurements.failed

    sources_train, sources_test, mag_train, mag_test = \
        train_test_split(measurements, mag, test_size=test_size,
    ...
    # add by YH.
    train_set = sources_train
    test_set = sources_test
    ...
    print("%.2f%% of the train set passed (>= 85)" %
          ((np.sum(train_set.final >= 85) / len(train_set)) * 100))
    print("%.2f%% of the test set are large earthquakes (<85)" %
          ((np.sum(test_set.final < 85) / len(test_set)) * 100))
...
def feature_engineering(feature, distance_correction=True):
    ...
    distance = feature.copy()['pretest']
    for col in feature.columns:
        if (col == 'pretest') | (col in feature_linear):
            continue
        elif col in feature_log:
            feature[col] = np.log10(feature[col].to_numeric())
        else:
            if distance_correction:
                feature[col] = np.log10(pd.to_numeric(feature[col])) + 2 * np.log10(distance)
            else:
                feature[col] = np.log10(feature[col].to_numeric())
    feature['pretest'] = np.log10(feature['pretest'])

    if not distance_correction:
        feature.drop(['pretest'], axis=1, inplace=True)
...
def make_feature_matrix_and_outcome(data_set):
    ...
    """
    outcome = data_set.copy()['final']
    feature = data_set.copy().drop(['final', 'failed'], axis=1)
...
def load_data():
    ...
    try:
        ...
        measurements = pd.read_csv(data_path + "learningMeasure.csv")
        train_set, test_set = split_train_and_test_set(measurements)  # just test_set have 0 rows within this function
...
def main(model, outputdir):
    ...
    train_ML_model(X_train, y_train, X_test, y_test, X_name,
                   ...
                   MI_flag=False, plot_flag=True, outputdir=outputdir)
